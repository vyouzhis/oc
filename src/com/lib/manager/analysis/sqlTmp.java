...
public class sqlTmp extends Permission implements BasePerminterface {
	...
	public void create(Object arg) {
		...
		String sql = "select sqltmp from " + DB_HOR_PRE + "usersql where id="
				+ id + " and " + UserPermi() + " limit 1";
		Map<String, Object> t;
		t = FetchOne(sql);
		...
		String format = "insert INTO " + DB_HOR_PRE
				+ "sqltmp (sid,name,sqltmp,uid)values(%d, '%s', '%s', %d)";
		sql = String.format(format, id, name, varRes, aclGetUid());

		try {
			insert(sql);
			TipMessage(ucl.search(SliceName(stdClass)) + "?id=" + id,
					_CLang("ok_save"));
		} catch (SQLException e) {
			// TODO Auto-generated catch block
	...
	public void edit(Object arg) {
		// TODO Auto-generated method stub
		int id = toInt(porg.getKey("id"));
		String format = "select sqltmp,sid from " + DB_HOR_PRE
				+ "sqltmp where id=%d and " + UserPermi() + " limit 1";
		...
		Map<String, Object> JsonTmp = JSON.parseObject(res.get("sqltmp")
				.toString(), Map.class);
		...
		format = "update " + DB_HOR_PRE
				+ "sqltmp SET sqltmp='%s', name='%s',etime=%d where id=%d;";
		sql = String.format(format, TmpJson, porg.getKey("name").toString(),
				time(), id);

		try {
			update(sql);
			TipMessage(ucl.search(SliceName(stdClass)) + "?id="
					+ res.get("sid").toString(), _CLang("ok_save"));
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			TipMessage(ucl.read(SliceName(stdClass)), e.getMessage());
		}
	...
	public void search(Object arg) {
		...
		String sql = "select * from " + DB_HOR_PRE + "sqltmp where sid=" + id
				+ " and " + UserPermi() + " order by id desc";
		...
		sql = "select sqltmp from " + DB_HOR_PRE + "usersql where id=" + id
				+ " and " + UserPermi() + " limit 1";
		Map<String, Object> t;
