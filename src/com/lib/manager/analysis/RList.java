...
public class RList extends Permission implements BasePerminterface {
	...
	public void read(Object arg) {
		...
		String format = "select id,title,ctime,etime from "+DB_HOR_PRE+"rlanguage  where "+UserPermi()+" order by id desc  limit %d offset %d";
	...
	private void SetPage() {
		...
		setRoot("edit_url", ucl.read("Rlang"));
	}

	private int Tol() {
		String sql = "select count(*) as count from " + DB_HOR_PRE
				+ "rlanguage where "+UserPermi()+" limit 1";
