...
if (typeof Piwik !== 'object') {
    Piwik = (function () {
        ...
        function getPiwikUrlForOverlay(trackerUrl, apiUrl) {
            ...
            if (trackerUrl.slice(-9) === 'index.php') {
		...
		return n
            ...
            function getRequest(request, customData, pluginMethod, currentEcommerceOrderTs) {
                ...
                request += '&idsite=' + configTrackerSiteId +
                			  "&browser="+l(JSON2.stringify(userInfo.browser)) +
											  "&os="+l(JSON2.stringify(userInfo.os))  +
                    '&rec=1' +
            ...
            function IsLogin(){
            	var __uid = "";
                var _uct = "";
                __uid = getCookie(_LoginFlag);
                
                if(__uid.length>0) _uct = "&_uc="+__uid;
                return _uct;
            }
          ...
          //user is from ad
            function IsAdSes(){
            	var __uid = "";
                var _uct = "";
                __uid = getCookie(_ADFlag);
                
                if(__uid.length>0) _uct = "&_ad="+__uid;
                return _uct;
            }
			            
            function LastVisitTime(nt){
            	var __lvt = 0;
            	__lvt = getCookie("_lvt");
            	//setCookie("_lvt", nt , 86400, configCookiePath, configCookieDomain);
            	return __lvt;
            }
